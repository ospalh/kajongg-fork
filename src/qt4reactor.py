...
from PyQt4.QtCore import QSocketNotifier, QTimer, QCoreApplication
...
class TwistedSocketNotifier(QSocketNotifier):
    ...
    def __init__(self, reactor, watcher, type):
        ...
        if type == QSocketNotifier.Read:
            ...
            self.fn = self.write
        self.activated.connect(self.fn)


    def shutdown(self):
        self.activated.disconnect(self.fn)
...
class QTReactor(PosixReactorBase):
    ...
    def setEnabled(self, enabled=True):
        for reader in self._reads.values():
            reader.setEnabled(enabled)
        for writer in self._writes.values():
